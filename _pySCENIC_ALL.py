#### pySCENIC
# 우선, source activate scanpy_1.8.1
...
test3 = test3[:, test3.var['highly_variable']]

row_attrs = {"Gene": np.array(test3.var_names) ,}
col_attrs = {"CellID": np.array(test3.obs_names),
             "nGene": np.array(np.sum(test3.X.transpose()>0 , axis=0)).flatten(),
             "nUMI": np.array(np.sum(test3.X.transpose(), axis=0)).flatten(),}
lp.create("/data/Projects/phenomata/01.Projects/11.Vascular_Aging/03.Scanpy/pySCENIC/ALL/test3.loom", test3.X.transpose(), row_attrs, col_attrs)

f_loom_path_scenic="/data/Projects/phenomata/01.Projects/11.Vascular_Aging/03.Scanpy/pySCENIC/test3.loom"
f_tfs="/data/Projects/phenomata/01.Projects/11.Vascular_Aging/03.Scanpy/pySCENIC/pySCENIC_Database/mm_mgi_tfs.txt"

!pyscenic grn {f_loom_path_scenic} {f_tfs} -o test3_adj.csv --num_workers 20
...
# 밑의 aucell을 --num_workers 30,40으로 하면 안 됨, 아래와 같은 에러 뜸
...
lf = lp.connect("/data/Projects/phenomata/01.Projects/11.Vascular_Aging/03.Scanpy/pySCENIC/ALL/test3_pyscenic_output.loom", mode='r+', validate=False)
lf.ca.keys()
...
umapDF = pd.DataFrame(test3.obsm['X_umap'], columns=['_X', '_Y'])
Embeddings_X = pd.DataFrame(index=lf.ca.CellID)
Embeddings_X = pd.concat([pd.DataFrame(test3.obsm['X_umap'], index=test3.obs.index)[0],
                          pd.DataFrame(test3.obsm['X_pca'], index=test3.obs.index)[0],
                          dr_umap['X']],
                          sort=False, axis=1, join='outer')
Embeddings_X.columns = ['1','2','3'] # '1': scanpy UMAP coordinate, '2': scanpy PCA coordinate, '3': pyscenic UMAP

Embeddings_Y = pd.DataFrame(index=lf.ca.CellID)
Embeddings_Y = pd.concat([pd.DataFrame(test3.obsm['X_umap'], index=test3.obs.index)[1],
                          pd.DataFrame(test3.obsm['X_pca'], index=test3.obs.index)[1],
                          dr_umap['Y']],
                          sort=False, axis=1, join='outer')
...
metaJson['metrics'] = [{"name": "nUMI"},
                       {"name": "nGene"},]
# {"name": "percent_mito"}
metaJson['annotations'] = [{"name": "Leiden_clusters", "values": list(set(test3.obs['leiden_r05'].astype(str)))},
                           {"name": "Timepoint", "values": list(set(test3.obs['batch'].values))},]


# pySCENIC regulon thresholds
metaJson["regulonThresholds"] = rt

for i in range(max(set([int(x) for x in test3.obs['leiden_r05']])) + 1): # range(0,6) ==> 0,1,2,3,4,5 (test3_endo end_leiden_r05)
clustDict = dict()
clustDict['id'] = i
clustDict['description'] = f'Cluster {i}' # 얘 i+1이 아니라 i라고 해야할 듯
metaJson['clusterings'][0]['clusters'].append(clustDict)


clusterings = pd.DataFrame()
clusterings["0"] = test3.obs['leiden_r05'].values.astype(np.int64)
...
col_attrs = dict(CellID = np.array(test3.obs.index),
                 nUMI = np.array(test3.obs['n_counts'].values),
                 nGene = np.array(test3.obs['n_genes'].values),
                 Leiden_clusters = np.array(test3.obs['leiden_r05'].values),
                 Timepoint = np.array(test3.obs['batch'].values),
                 Embedding = dfToNamedMatrix(umapDF),
                 Embeddings_X = dfToNamedMatrix(Embeddings_X),
                 Embeddings_Y = dfToNamedMatrix(Embeddings_Y),
                 RegulonsAUC = dfToNamedMatrix(auc_mtx),
                 Clusterings = dfToNamedMatrix(clusterings),
                 ClusterID = np.array(test3.obs['leiden_r05'].values))
...
lp.create(filename="test3_pyscenic_integrated_output.loom",
          layers=lf[:,:],
          row_attrs=row_attrs,
          col_attrs=col_attrs,
          file_attrs=attrs)
...
# pySCENIC output
lf = lp.connect("test3_pyscenic_integrated_output.loom", mode='r', validate=False )
meta = json.loads(zlib.decompress(base64.b64decode( lf.attrs.MetaData)))
